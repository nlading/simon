...
class RectButton:
	def __init__(
		...
		"""
		Manages the appearance and interactions of rectangular buttons in a GUI created with pygame.
		:param surface: The window in which this object will be drawn.
		:param location: The location and dimensions of the object as allowed by pygame.draw.rect
		:param on_color: RGB tuple defining the color of the object in its active state
		:param off_color: RGB tuple defining the color of the object in its inactive state
		...
		self.tone = tone
		self.on_timer = 0               # Integer length of time object has been active in number of surface refreshes
	...
	def set_to_base(self):
		"""
		Draws the button on the surface in the state it was instantiated in
		:return:
		"""
	...
	def center_text(self):
		"""
		Determines where the text must be drawn to share a center point with the button
		:return:
		"""
	...
	def set_text(self, text):
		"""
		Allows outside methods to set the text of the button
		:param text: String to be drawn on the button
		:return:
		"""
		self.text = text
	...
	def extinguish(self):
		"""
		Sets the button to its 'off' state.
		:return:
		"""
		pygame.draw.rect(self.surface, self.off_color, self.location)
		self.center_text()
	...
	def ignite(self):
		"""
		Sets the button to its 'on' state.
		:return:
		"""
		pygame.draw.rect(self.surface, self.on_color, self.location)
		self.center_text()
	...
	def check_clicked(self, click):
		"""
		Used by external methods to verify if mouse click occurred within the button's location
		:param click: Pygame event generated by a mouse click
		:return: True if click exists within button's location. False otherwise.
		"""
	...
	def update(self):
		"""
		Manages the time the button is active and re-draws the button and text to the surface.
		:return:
		"""
